  ...
  try {
    ...
    for (const event of allCreates) {
      ...
      const createData = {
        ...
        // Cost fields will be added below
        costETH: "0.0",
        costTitanX: "0.0",
        rawCostETH: "0",
        rawCostTitanX: "0",
    ...
    // For creates, we need to get cost from positions
    cachedData.stakingData.createEvents.forEach(event => {
      ...
      if (userPos) {
        ...
        if (matchingPosition) {
          ...
          event.stakingDays = matchingPosition.stakingDays.toString();
          ...
          console.log(`  ⚠️  No position match for create by ${event.user.slice(0,10)}... stakeIndex: ${event.stakeIndex}`);
        }
      }
    });
