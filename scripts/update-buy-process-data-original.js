...
async function updateBuyProcessData() {
  ...
  try {
    ...
    // Calculate total TORUS from new burns for ETH allocation
    let totalNewTorusBurned = 0;
    const burnsByDate = {};
    ...
    for (const event of newBuyAndBurnEvents) {
      ...
      burnsByDate[dateKey].push({
        event: event,
        torusBurned: torusBurned
      });
      totalNewTorusBurned += torusBurned;
    }
    
    // Process burns with proportional ETH allocation if there's new ETH
    for (const [dateKey, burns] of Object.entries(burnsByDate)) {
      ...
      for (const burn of burns) {
        ...
        newDailyData[dateKey].torusBurned += burn.torusBurned;
        const titanXAmount = parseFloat(ethers.utils.formatEther(burn.event.args.titanXAmount));
        ...
        const tx = await provider.getTransaction(burn.event.transactionHash);
        const functionSelector = tx.data.slice(0, 10);
        
        if (functionSelector === '0x39b6ce64') {
          // ETH burn - get WETH deposit amount
          const receipt = await provider.getTransactionReceipt(burn.event.transactionHash);
